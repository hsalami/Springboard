...
SELECT Bookings.bookid, Facilities.name AS facilityname, CONCAT(Members.firstname,' ',Members.surname) AS membername,
...
SELECT t2.bookid, t2.name AS "facilityname", concat(Members.firstname,' ',Members.surname) AS membername, t2.cost
FROM Members
JOIN (
...
SELECT t2.name AS facilityname, SUM(t2.cost) AS total
FROM Members
JOIN (
       SELECT Bookings.bookid, Facilities.name, Bookings.memid,
              CASE WHEN Bookings.memid =0 THEN Facilities.guestcost * Bookings.slots 
